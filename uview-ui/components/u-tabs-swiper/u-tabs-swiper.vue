		...
		<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation :style="{ zIndex: zIndex + 1 }">
			<view class="u-tabs-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
				<view class="u-tabs-item" :style="[tabItemStyle(index)]"
				 v-for="(item, index) in getTabs" :key="index" :class="[preId + index, {'u-tabs-line': !showBar && current === index}]" @tap="emit(index)">
...
<script>
	 ...
	 * @property {String} active-color 滑块和激活tab文字的颜色（默认#01B3F4）
		...
		methods: {
			...
			getTabsInfo() {
				return new Promise((resolve, reject) => {
					...
					view.exec(res => {
						if (!res[0] && typeof res[0] != "undefined" && res[0] != 0) {
							// 进入判断说明返回的 res 的元素为 null，暂时不知道怎么避免
							this.$emit('update:showBar', false)
						} else {
							const arr = [];
