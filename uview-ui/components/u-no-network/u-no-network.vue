	...
	<view class="u-no-network" v-if="!isConnected" :style="{'z-index': uZIndex}" @touchmove.stop.prevent="() => {}">
		<view class="u-inner">
			...
			<view class="u-to-setting">
				{{i18n.checkNetwork}}<text class="u-setting-btn" @tap="openSettings">{{i18n.setting}}</text>
			</view>
			<!-- #endif -->
			...
			retry() {
				...
				uni.getNetworkType({
					success: (res) => {
						...
						if (res.networkType == 'none') {
							uni.showToast({
								title: this.$u.$t.t('commonText.noNetwork'),
								icon: 'none',
								position: 'top'
							})
							this.isConnected = false;
							...
							uni.showToast({
								title: this.$u.$t.t('commonText.networkLinked'),
								icon: 'none',
								position: 'top'
							})
							this.isConnected = true;
